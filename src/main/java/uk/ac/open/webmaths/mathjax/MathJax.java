...
public class MathJax
{
	...
	/** Amount to scale the equation by. I just eyeballed this, comparing results for a few values. */
	public static final double EX_ZOOM_FACTOR = 1.04;
	...
	/** Matches a value in ex, as the whole attribute or part of a style attribute */
	private final static Pattern REGEX_EXVALUE = Pattern.compile(
		"(?<![0-9-])(-?[0-9]+(\\.[0-9]+))ex(?=;|$| )");
	...
	/**
	 * If there is an 'ex' value in the specified attribute, scales it by the ex zoom factor.
	 * @param e Element
	 * @param attribute Attribute name
	 */
	protected static void scaleEx(Element e, String attribute)
	{
		String value = e.getAttribute(attribute);
		Matcher m = REGEX_EXVALUE.matcher(value);
		if (m.find())
		{
			e.setAttribute(attribute,
				value.substring(0, m.start()) +
				round(Double.parseDouble(m.group(1)) * EX_ZOOM_FACTOR) + "ex" +
	 ...
	 * @throws IOException Other error
	 */
	public String getSvg(InputEquation eq, boolean correctBaseline, double exSize, String rgb, boolean compatibilityScale)
		throws MathJaxException, IOException
	{
		...
		try
		{
			...
			// Scale the equation slightly. This is to preserve behaviour from earlier MathJax
			// version which rendered things slightly larger in ex.
			if (compatibilityScale)
			{
				scaleEx(root, "width");
				scaleEx(root, "height");
				scaleEx(root, "style");
			...
			// We now correct baseline only if converting to pixels.
			if(correctBaseline)
			{
				// Get the width and height in ex.
				String heightStr = root.getAttribute("height");
				String widthStr = root.getAttribute("width");
				if (!widthStr.endsWith("ex") || !heightStr.endsWith("ex"))
				{
					throw new IOException("Unexpected SVG format (width or height not in ex): " +
						widthStr + ", " + heightStr);
				}
				double widthEx, heightEx;
				try
				{
					widthEx = Double.parseDouble(widthStr.substring(0, widthStr.length() - 2));
					heightEx = Double.parseDouble(heightStr.substring(0, heightStr.length() - 2));
				}
				catch (Exception e)
				{
					throw new IOException("Unexpected SVG format (width or height ex values not parsed): " +
							widthStr + ", " + heightStr);
				}
				...
				if (convertToPixels)
				{
					// Make this an exact number of pixels by slightly increasing the height of the equation.
