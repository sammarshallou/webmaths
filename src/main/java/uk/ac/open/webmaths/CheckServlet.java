...
import static uk.ac.open.webmaths.StatusServlet.esc;
...
import uk.ac.open.webmaths.mathjax.*;
...
{
	private static final long serialVersionUID = 234334234553215L;
	 ...
	 * @param equation Equation to render
	 * @return True if it failed, false if it is OK (or OK for now)
	 */
	private boolean mathJaxCheck(StringBuilder out, String baseUrl, SourceEquation equation)
	{
		synchronized(mathJaxCheckSynch)
		{
			...
			mathJaxCheck = new MathJaxChecker(baseUrl, equation);
			try
			{
	...
	private class MathJaxChecker extends Thread
	{
		...
		private SourceEquation sourceEquation;
		 ...
		 */
		public MathJaxChecker(String baseUrl, SourceEquation sourceEquation)
		{
			super("MathJax checker");
			this.baseUrl = baseUrl;
			this.sourceEquation = sourceEquation;
			start();
		}
			...
			try
			{
				...
				synchronized(mathJaxCheckSynch)
				{
					...
					if(first.isOk())
					{
						...
						result = getPngImage(first.getPng());
					}
					else
					{
						error = esc(first.getError());
					}
				}
			}
			catch(Throwable t)
